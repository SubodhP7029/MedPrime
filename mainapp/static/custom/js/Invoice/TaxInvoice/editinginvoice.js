...
$(document).ready(function () {
    ...
    creator = $('#idofcreator').val()
    loggedinuser = $('#idofuser').val()
    invoiceid = $('#invoiceeid').val()
    ...
    $.get("/invoicegetallprodcol/", function (data) {
        ...
        $.get("/invoicegetallprod/", function (data) {
            ...
            SelectedCustomerUsername = "SELECT billingaddress,billingpincode,billingcity,billingstate,billingcountry,gst,stateid,address, pincode,  city,state,  country ,customername FROM public.mainapp_customerprofile WHERE id= " + selectedcustid
    ...
    $.get("/addtoinvoicedb/", { sqlParam: addtoinvoicedb }, function (data) {
        ...
        $.get("/editinvoices/", { sqlParam: editQuery }, function (data) {
            ...
            Swal.fire({
                position: 'top-end',
                icon: 'success',
                title: 'Invoice number ' + invoiceid + ' Updated Successfully!',
                showConfirmButton: false,
                timer: 2000
            })
            $('#redirectToHere')[0].click();
        })
