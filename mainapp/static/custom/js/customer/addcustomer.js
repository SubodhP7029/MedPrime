...
//check if country in India in Shipping
function stateandcitydropinshipping() {
    if ($('#id_country').val() == 'India') {
        document.getElementById('typeofInput_state').innerHTML = '<select onchange="print_city(\'id_city\', this.selectedIndex,\'id_state\');" name="state" class="select form-control form-control" id="id_state"></select>'
        document.getElementById('typeofInput_city').innerHTML = ' <select name="city" class="select form-control" id="id_city">'
        print_state("id_state");
        // $('#id_city').select2();
        // $('#id_state').select2();
    } else {
        document.getElementById('typeofInput_state').innerHTML = ' <input type="text" onkeyup="getStateid()" name="state" placeholder="Shipping State" class="textinput form-control" id="id_state">'
        document.getElementById('typeofInput_city').innerHTML = '<input type="text" name="city" placeholder="Shipping City" class="textinput form-control" id="id_city">'
    }
}
//all cities
...
function print_city(city_id, city_index, state_id) {
...
$(document).ready(function () {
    ...
    var countrylist1 = document.getElementById('id_billingcountry')
    var countrylist2 = document.getElementById('id_country')

    for (i = 0; i < country_list.length; i++) {
        var op = document.createElement('option')
        op.value = country_list[i]
        op.innerText = country_list[i]
        if (country_list[i] == 'India') {
            op.selected = true
        }
        countrylist1.appendChild(op)
        var op2 = op.cloneNode(true);
        countrylist2.appendChild(op2)
    }
...
$("#id_username").on("keyup", function () {
    ...
    uniqueChecker(string, 'username', input.length)
});

$("#for_email").on("keyup", function () {
    var input = $("#id_email").val()
    var string = "SELECT email FROM public.auth_user where email = '" + input + "'"
    uniqueChecker(string, 'email', input.length)
});
function uniqueChecker(sql, field, length) {

    document.getElementById('for_' + field + '').className = 'form-group has-warning has-feedback'
    document.getElementById('for_' + field + '_symbol').className = 'ti-reload form-control-feedback'

    $.get("/checkUserName/", { sqlParam: sql }, function (data) {
        if (data.length != 0) {
            document.getElementById('for_' + field + '').className = 'form-group has-error has-feedback'
            document.getElementById('for_' + field + '_symbol').className = 'ti-close form-control-feedback'

        } else {
            if (length > 7) {
                document.getElementById('for_' + field + '').className = 'form-group has-success has-feedback'
                document.getElementById('for_' + field + '_symbol').className = 'ti-check form-control-feedback'
            } else {
                document.getElementById('for_' + field + '').className = 'form-group has-error has-feedback'
                document.getElementById('for_' + field + '_symbol').className = 'ti-close form-control-feedback'
